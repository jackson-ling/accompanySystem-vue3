    ...
    <div class="content">
      <div class="menu-group">
        <div class="menu-item" @click="handleChangeAvatar">
          ...
          <div class="right">
            <el-avatar :size="32" :src="getUserAvatar(userStore.userInfo?.avatar)" />
...
const submitNicknameChange = async () => {
  ...
  if (userStore.userInfo) {
    try {
      // 调用API更新用户资料
      await updateUserProfile({ nickname: newNickname.value })
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = async (e) => {
      if (e.target?.result && userStore.userInfo) {
        ...
        try {
          // 调用API更新头像
          await updateUserAvatar({ avatar: avatarBase64 })
          userStore.userInfo.avatar = avatarBase64
